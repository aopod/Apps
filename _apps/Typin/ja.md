---
layout: app_Typin
lang: ja
permalink: /Typin/ja/
title: "Typin"
app_title: "Typin"
app_tagline: "iPhoneで美しいMarkdown編集を"
features_title: "強力な機能"
features_subtitle: "生産的なMarkdownライティングに必要なすべて"
cta_title: "執筆を始めよう"
cta_text: "Typinをダウンロードして、美しいMarkdownドキュメントの作成を始めましょう。"
download_button: "App Storeでダウンロード"
wechat_overlay_title: "システムブラウザで開く"
wechat_overlay_message: "アプリをダウンロードするには、Safariまたはシステムブラウザでこのページを開いてください。"
wechat_step_1: "右上の共有ボタンをタップ"
wechat_step_2: "下にスクロールして'ブラウザで開く'をタップ"
wechat_close_button: "了解しました"
floating_anchor_label: "ダウンロードへ"
mobile_banner_title: "Typin"
mobile_banner_subtitle: "App Storeで無料"
mobile_banner_cta: "ダウンロード"
---

<div class="feature-category">
    <h2>編集機能</h2>
    <div class="features-grid">
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
            </div>
            <h3>デュアルモード編集</h3>
            <p>ワンタップでプレーンテキストとMarkdownレンダリングを切り替え。好みの編集モードを記憶し、カスタムレンダラーとシンタックスハイライトをサポート。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg>
            </div>
            <h3>Markdownサポート</h3>
            <p>見出し、太字、斜体、インラインコード、リスト、引用を完全サポート。カスタムレンダラーはリアルタイムプレビュー、スマートリスト継続、ジェスチャーによる見出しレベル調整をサポート。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M20.71 7.04c.39-.39.39-1.04 0-1.41l-2.34-2.34c-.37-.39-1.02-.39-1.41 0l-1.84 1.83 3.75 3.75M3 17.25V21h3.75L17.81 9.93l-3.75-3.75L3 17.25z"/></svg>
            </div>
            <h3>スマートシンボル挿入</h3>
            <p>執筆のリズムを崩さず、一般的なMarkdownシンボルを素早く挿入。ブロックシンボルは行頭に、インラインシンボルはカーソル位置に挿入。連続タップで見出しレベルをアップグレード。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>
            </div>
            <h3>テキスト操作</h3>
            <p>フォーカス状態に応じたスマートコピー：フォーカスなしで全コピー、フォーカスありでオプションメニュー表示。現在の行のコピー、選択、貼り付け、置換、ドキュメントのクリアをサポート。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 14c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H9V5h6v2z"/></svg>
            </div>
            <h3>クリップボード通知</h3>
            <p>バックグラウンドから復帰時にクリップボードの変化を自動検出し、スマートに貼り付けオプションを表示。エディターのフォーカス状態に応じて貼り付け、現在の行を置換、末尾に追加の操作を提供。長押しで空行削除もサポート。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M9.5 6.5v3h-3v-3h3M11 5H5v6h6V5zm-1.5 9.5v3h-3v-3h3M11 13H5v6h6v-6zm6.5-6.5v3h-3v-3h3M19 5h-6v6h6V5zm-6 8h1.5v1.5H13V13zm1.5 1.5H16V16h-1.5v-1.5zM16 13h1.5v1.5H16V13zm-3 3h1.5v1.5H13V16zm1.5 1.5H16V19h-1.5v-1.5zM16 16h1.5v1.5H16V16zm1.5-1.5H19V16h-1.5v-1.5zm0 3H19V19h-1.5v-1.5zM19 13h-1.5v1.5H19V13z"/></svg>
            </div>
            <h3>QRコードスキャン</h3>
            <p>内蔵QRコードスキャナーで、スキャンした内容を素早くドキュメントに挿入。フォーカス時はカーソル位置に、フォーカスなし時は末尾に追加。コピー＆保存操作もサポート。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M4 10.5c-.83 0-1.5.67-1.5 1.5s.67 1.5 1.5 1.5 1.5-.67 1.5-1.5-.67-1.5-1.5-1.5zm0-6c-.83 0-1.5.67-1.5 1.5S3.17 7.5 4 7.5 5.5 6.83 5.5 6 4.83 4.5 4 4.5zm0 12c-.83 0-1.5.68-1.5 1.5s.68 1.5 1.5 1.5 1.5-.68 1.5-1.5-.67-1.5-1.5-1.5zM7 19h14v-2H7v2zm0-6h14v-2H7v2zm0-8v2h14V5H7z"/></svg>
            </div>
            <h3>見出しナビゲーション</h3>
            <p>Markdownモードでドキュメントの見出しアウトラインを素早く表示。タップで該当位置にジャンプ。H1-H6の複数レベルをサポートし、レベルごとに色分け表示。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M2.5 4v3h5v12h3V7h5V4h-13zm19 5h-9v3h3v7h3v-7h3V9z"/></svg>
            </div>
            <h3>文字数カウント</h3>
            <p>リアルタイムで文字数をカウントし、トップツールバーに表示。複数言語の文字数計算をサポートし、ドキュメント共有時に単位を自動的に適応。</p>
        </div>
    </div>
</div>

<div class="feature-category">
    <h2>ファイル管理</h2>
    <div class="features-grid">
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 9H9V9h10v2zm-4 4H9v-2h6v2zm4-8H9V5h10v2z"/></svg>
            </div>
            <h3>マルチドキュメント対応</h3>
            <p>複数のドキュメントを作成、切り替え、管理。最後に開いたドキュメントを記憶。重複タイトル検出で整理整頓、自動生成された一意のタイトルを保証。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M20 6h-8l-2-2H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm0 12H4V8h16v10z"/></svg>
            </div>
            <h3>ドキュメント操作</h3>
            <p>ドキュメントの名前変更、削除、ピン留め、共有を簡単に。.mdまたは.txt形式でのエクスポートをサポート、変更時刻とピン状態でソート。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M17 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H5V5h10v4z"/></svg>
            </div>
            <h3>ドキュメントメタデータ</h3>
            <p>ドキュメントの作成時刻、変更時刻、文字数、プレビュー内容を自動的に追跡。相対時刻表示はアプリ言語に基づいて自動フォーマット。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
            </div>
            <h3>ゴミ箱</h3>
            <p>削除したドキュメントは完全に削除されずゴミ箱に移動し、いつでも復元可能。ゴミ箱で削除ドキュメントの一覧を表示し、個別の復元や完全削除、ゴミ箱を空にする操作をサポート。</p>
        </div>
    </div>
</div>

<div class="feature-category">
    <h2>リアルタイム同期</h2>
    <div class="features-grid">
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
            </div>
            <h3>TypinSyncモジュール</h3>
            <p>HTTPサーバーベースのローカルネットワークリアルタイムテキスト同期。オプションのパスワード認証をサポート。データはローカルネットワーク内に留まり、任何のサーバーにもアップロードされません。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-5 14H9v-2h6v2zm5-7H4V6h16v5z"/></svg>
            </div>
            <h3>Webクライアント</h3>
            <p>多言語表示と適応的なライト/ダークモードを備えたレスポンシブWebクライアント。自動接続検出、リアルタイムテキスト更新、ページ非表示時はポーリングを一時停止。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
            </div>
            <h3>同期ステータスインジケーター</h3>
            <p>トップツールバーに呼吸アニメーション付きで接続状態を表示。タップしてアクセスURL、プライバシー通知、クイックアクションを含むサーバー情報ポップアップを表示。</p>
        </div>
    </div>
</div>

<div class="feature-category">
    <h2>ユーザーエクスペリエンス</h2>
    <div class="features-grid">
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M20 5H4c-1.1 0-1.99.9-1.99 2L2 17c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm-9 3h2v2h-2V8zm0 3h2v2h-2v-2zM8 8h2v2H8V8zm0 3h2v2H8v-2zm-1 2H5v-2h2v2zm0-3H5V8h2v2zm9 7H8v-2h8v2zm0-4h-2v-2h2v2zm0-3h-2V8h2v2zm3 3h-2v-2h2v2zm0-3h-2V8h2v2z"/></svg>
            </div>
            <h3>キーボード管理</h3>
            <p>動的ツールバー配置によるスマートキーボード処理。ツールバーはキーボードの上に浮き、Markdownシンボルへのクイックアクセスを提供、キーボード表示時に自動的に上昇。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9 9-4.03 9-9c0-.46-.04-.92-.1-1.36-.98 1.37-2.58 2.26-4.4 2.26-2.98 0-5.4-2.42-5.4-5.4 0-1.81.89-3.42 2.26-4.4-.44-.06-.9-.1-1.36-.1z"/></svg>
            </div>
            <h3>テーマ対応</h3>
            <p>ライト、ダーク、システムテーマから選択。動的な色は滑らかなトランジションでシームレスに適応、エディターと設定をリアルタイムで更新。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M9 11.24V7.5C9 6.12 10.12 5 11.5 5S14 6.12 14 7.5v3.74c1.21-.81 2-2.18 2-3.74C16 5.01 13.99 3 11.5 3S7 5.01 7 7.5c0 1.56.79 2.93 2 3.74zm9.84 4.63l-4.54-2.26c-.17-.07-.35-.11-.54-.11H13v-6c0-.83-.67-1.5-1.5-1.5S10 6.67 10 7.5v10.74l-3.43-.72c-.08-.01-.15-.03-.24-.03-.31 0-.59.13-.79.33l-.79.8 4.94 4.94c.27.27.65.44 1.06.44h6.79c.75 0 1.33-.55 1.44-1.28l.75-5.27c.01-.07.02-.14.02-.2 0-.62-.38-1.16-.91-1.38z"/></svg>
            </div>
            <h3>触覚フィードバック</h3>
            <p>滑らかなアニメーションと反応の良いタッチフィードバックをお楽しみください。ツールバーボタンタップ、ピン操作、その他の操作が触覚フィードバックを提供して体験を向上。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12.87 15.07l-2.54-2.51.03-.03c1.74-1.94 2.98-4.17 3.71-6.53H17V4h-7V2H8v2H1v2h11.17C11.5 7.92 10.44 9.75 9 11.35 8.07 10.32 7.3 9.19 6.69 8h-2c.73 1.63 1.73 3.17 2.98 4.56l-5.09 5.02L4 19l5-5 3.11 3.11.76-2.04zM18.5 10h-2L12 22h2l1.12-3h4.75L21 22h2l-4.5-12zm-2.62 7l1.62-4.33L19.12 17h-3.24z"/></svg>
            </div>
            <h3>多言語対応</h3>
            <p>7つの言語に対応：簡体字中国語、繁体字中国語、英語、日本語、韓国語、ドイツ語、フランス語。再起動なしで即座に切り替え。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M17 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H5V5h10v4z"/></svg>
            </div>
            <h3>自動保存</h3>
            <p>設定可能な自動保存間隔（10秒、30秒、60秒、120秒）で作業を失いません。アプリがバックグラウンドに入ると自動保存、保留中のドキュメント追跡で不要な保存を回避。</p>
        </div>
        <div class="feature-card">
            <div class="feature-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
            </div>
            <h3>スクロール位置記憶</h3>
            <p>各ドキュメントのスクロール位置を自動的に記憶。ドキュメントを切り替えても、戻った時に前回の読書位置を復元。ドキュメントIDごとに独立してデータを管理。</p>
        </div>
    </div>
</div>
